$(document).ready(function() { 
 ...
 $("#feed_submit").live('click', function(){
			...
			var obj_ustream = {};
            obj_ustream.name = "ustream";
            obj_ustream.value = ustream;
            json_array.push(obj_ustream);
			
			var obj_youtube = {};
            obj_youtube.name = "youtube";
            obj_youtube.value = youtube;
            json_array.push(obj_youtube);
			
			var obj_instagram = {};
            obj_instagram.name = "instagram";
            obj_instagram.value = instagram;
            json_array.push(obj_instagram);
   ...
   $("#text_color_div").colorpicker().on('changeColor', function(ev){
    ...
    $("#submit_contact").live('click', function(){
       var App_ID = $("#App_ID").val();
		...
		var json_genre = {};
        json_genre.value = $("#genre").val();
        json_genre.name = "music_type";
        json_array.push(json_genre);
...
var send = "json=" + JSON.stringify(json_array);
       ...
       $('input[id^=web_ext]').live('change', function() { 
         	var App_ID = $("#App_ID").val();
			var web_ext = $("#web_ext").val();
		
				if( web_ext.indexOf( " " ) !== -1 )
				{	
					alert('No Spaces Allowed For Your Website');
					$("#picture_bad").css("display", "block");
                    $("#picture_bad").html("No Spaces Allowed For Your Website");
            ...
            var send = "website_ext=" + web_ext;
          
            $.ajax({
                    url: "/entertainment/web_ext2/" + App_ID  ,
                    type: 'POST',
                    dataType: 'text',
                    data: send,
                    beforeSend: function () {
            ...
                    success: function (data) 
                    {
                            ...
                            {
                              $("#picture_bad").css("display", "none");
							  $("#picture_good").html("The web extension available and has been saved.");
							  $("#picture_good").css("display", "visible");
                            }
                            else{ 
								$("#picture_bad").css("display", "block");
								$("#picture_bad").html("Sorry, the web extension is already in use.");
								$("#picture_good").css("display", "none");
                            }
...
function gotoSite()
{
	...
	var website = document.getElementById('web_ext').value;
	window.open("http://"+website+".shwcase.co");
}

function submitapp()
{
	var App_ID =document.getElementById('App_ID').value;
	window.location = "/process/submit2/"+App_ID;
}